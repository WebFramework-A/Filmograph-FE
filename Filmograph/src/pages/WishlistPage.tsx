        ...
        <div className="min-h-screen bg-[#0d5a5a] text-white p-8 pt-24">
            <div className="max-w-6xl mx-auto">
                <div className="flex items-center gap-4 mb-8 border-b border-white/20 pb-4">
                    ...
                    <h1 className="text-4xl font-bold text-[#FFD700]">찜한 영화 전체 목록</h1>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {likes.map((movie) => (
                        <div key={movie.id} className="relative group cursor-pointer" onClick={() => navigate(`/detail/${movie.id}`)}>
                            ...
                            <p className="mt-2 text-center font-bold truncate text-white/90">{movie.title}</p>
                        </div>
                    ))}
                </div>
            </div>
