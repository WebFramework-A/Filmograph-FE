...
const GraphDescription = () => {
  ...
  return (
    <div className="min-h-screen snap-start flex flex-col items-center pt-50 bg-[#fdf8e8]">
      <div className="text-5xl mb-5 text-[#0b4747]">그래프 종류</div>

      <button
        className="border border-[#0b4747]/50 rounded-4xl hover:bg-[#0b4747]/50 hover:text-white duration-400 px-4 py-2 mb-10 cursor-pointer transition-colors"
      ...
      <div className="grid grid-cols-3 w-full max-w-7xl px-10 py-5 gap-10 items-start">
        {/* 좌측 (리스트 부분) */}
        <div className="col-span-2 flex flex-col text-[#8c8c83]">
          {/* 위에 테두리선 */}
          <div className="w-full border-t border-[#b5b5a9]"></div>
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div className="flex items-center py-4 px-2 transition-colors duration-300 relative z-10">
                {/* 화살표 애니메이션 배경 */}
                <div className="absolute left-35 right-0 h-px bg-black opacity-0 -translate-x-10 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-500 ease-out flex items-center">
                  <span className="absolute right-0 text-2xl text-black -mt-1">
                    &gt;
                  </span>
                </div>

                {/* 숫자 */}
                <span className="text-5xl font-serif italic tracking-tighter mr-16 text-[#b5b5a9] group-hover:text-black transition-colors duration-300">
                  {item.id}
                </span>

                {/* 텍스트 */}
                <span className="text-4xl text-[#b5b5a9] group-hover:text-black transition-colors duration-300 bg-[#fdf8e8] pr-4 z-20 ">
                  {item.text}
                </span>
              </div>

              {/* 하단 테두리 */}
              <div className="w-full border-b border-[#b5b5a9]"></div>
            </div>
          ))}
        </div>
