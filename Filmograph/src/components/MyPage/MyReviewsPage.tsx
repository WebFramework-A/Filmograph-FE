...
export default function MyReviewsPage() {
    ...
    return (
        <div className="min-h-screen bg-[#0d5a5a] text-white pt-24 px-8">
            <div className="max-w-4xl mx-auto">
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <h1 className="text-3xl font-bold text-yellow-200">내가 쓴 리뷰 목록</h1>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="grid gap-6">
                        {reviews.map((review) => (
                            <div key={review.id} className="bg-black/20 p-6 rounded-xl border border-white/10 hover:border-yellow-400/50 transition relative group">
                                {/* 상단 영역 */}
                                        ...
                                        <div className="flex items-center gap-2 text-sm text-white/40">
                                            <span>
                                                {review.createdAt?.seconds
                                                    ? new Date(review.createdAt.seconds * 1000).toLocaleDateString() + " 작성"
                                                    : ""}
                                            </span>
                                            {review.isAnonymous && <span className="text-xs bg-white/10 px-1 rounded">익명</span>}
                                        </div>
                                    </div>
