...
import type { GraphT, Link, LinkT, NodeT } from "../../types/graph";
...
export default function BipartiteGraph({
  ...
  // 데이터 로드 후 5초가 지나도 그래프가 준비 안 되면 강제로 보여줌
  useEffect(() => {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full relative flex flex-col items-center justify-center bg-[#0b4747]"
    >
      {/* 로딩 오버레이 */}
      {(!data || !isGraphReady) && (
      ...
      {data && dimensions.width > 0 && (
        <ForceGraph2D
          ...
          onNodeClick={(node: any) => {
            ...
            fgRef.current?.zoomToFit(500, 50, (n: any) =>
          ...
          nodeCanvasObject={(rawNode, ctx, globalScale) => {
            ...
            const scaleAdjustment =
              globalScale > 1 ? Math.sqrt(globalScale) : 1;
      ...
      {selectedNode && selectedNode.type === "movie" && (
        <div className="absolute top-4 right-4 w-72 bg-black/80 text-white p-5 rounded-xl border border-white/20 shadow-2xl backdrop-blur-md z-50 animate-fade-in">
          ...
          {/* 타입 표시 */}
          <p className="text-sm text-white/60 mb-4 capitalize">영화 (Movie)</p>
